...
LLM 2 (Visual Designer): 接收结构化数据，实时构建符合审美、可交互的 HTML 代码。 Chatflow 截图
实时渲染：配合 Dify Artifacts 功能，实现书籍卡片的一键生成与即时预览。
自动化流：无需手动查资料，只需输入一个书名。
⚙️ 工作流逻辑 (Chatflow)
输入节点: 用户输入想要搜索的书籍名称。
搜索节点: 第一个 LLM 调用搜索工具（如 Google/Bing）定位书籍元数据。
转换节点: 整理搜索结果为 JSON 结构。
生成节点: 第二个 LLM 填充 HTML/Tailwind CSS 模板。
输出预览: 通过 Artifacts 渲染最终视觉卡片。
🚀 如何使用
在 Dify 中导入本项目提供的 DSL 文件。
工作流文件: 点击下载 小红书读书卡片.yml
配置你的搜索工具 API 密钥（如 Serper 或 Google Search）。
在对话框输入书名，点击运行。
⚠️ 常见问题排查
